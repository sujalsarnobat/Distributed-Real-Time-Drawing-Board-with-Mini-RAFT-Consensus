...
const wsHost          = location.port === '8080' ? `${location.hostname}:3000` : location.host;
const GATEWAY_WS_URL  = `${wsProtocol}//${wsHost}`;
let ws                = null;
...
function startHealthPolling() {
  ...
  const httpHost   = location.port === '8080' ? `${location.hostname}:3000` : location.host;
  const HEALTH_URL = `${location.protocol}//${httpHost}/health`;
